...
import { ImageStudio, VideoStudio, ClippingStudio, VibeMotionStudio, LipSyncStudio, RecastStudio, CinemaStudio, AudioStudio, MarketingStudio, WorkflowStudio, AgentStudio, AppsStudio, FilmAgentStudio, StoryboardStudio, ScriptsStudio, ReelsStudio, VideoToolsStudio, getUserBalance } from 'studio';
...
export default function StandaloneShell() {
  ...
  // Cross-module navigation from the Cascade suite (e.g. Scripts/Storyboard → Film Agent)
  useEffect(() => {
    const onGoto = (e) => {
      const tab = e?.detail?.tab;
      if (tab && TABS.find(t => t.id === tab)) handleTabChange(tab);
    };
    window.addEventListener('cascade:goto', onGoto);
    return () => window.removeEventListener('cascade:goto', onGoto);
  }, []);
  ...
  const fetchBalance = useCallback(async (key) => {
    try {
      ...
      // Non-fatal (e.g. invalid/expired key) — keep it quiet so it doesn't surface
      // in the dev error overlay.
      console.debug('Balance fetch failed:', err?.message || err);
    }
  }, []);
  ...
  return (
    ...
    >
      {/* Drag Overlay */}
      {isDragging && (
        <div className="fixed inset-0 z-[100] bg-[#d9ff3f]/10 backdrop-blur-md border-4 border-dashed border-[#d9ff3f]/50 flex items-center justify-center pointer-events-none transition-all duration-300">
          <div className="bg-[#0a0a0a] p-8 rounded-3xl border border-white/10 shadow-2xl flex flex-col items-center gap-4 scale-110 animate-pulse">
            <div className="w-20 h-20 bg-[#d9ff3f] rounded-2xl flex items-center justify-center">
      ...
      {isHeaderVisible && (
        <header className="flex-shrink-0 h-14 border-b border-white/[0.03] flex items-center justify-between px-6 bg-black/20 backdrop-blur-md z-40 gap-4">
          ...
          <div className="flex-1 min-w-0 mx-4 sm:mx-6 relative overflow-hidden h-full flex items-center justify-start lg:justify-center">
            ...
            <nav className="flex items-center gap-0.5 overflow-x-auto hide-scrollbar w-full lg:w-auto px-4 lg:px-0">
              {TABS.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => handleTabChange(tab.id)}
                  className={`text-[13px] font-medium transition-colors duration-150 whitespace-nowrap px-3 py-1.5 rounded-lg flex-shrink-0 ${
                    activeTab === tab.id
      ...
      {/* Studio Content */}
      <div className="flex-1 min-h-0 relative overflow-hidden openfield-studio">
        ...
        {activeTab === 'video-tools' && <VideoToolsStudio apiKey={apiKey} />}
        {activeTab === 'film-agent' && <FilmAgentStudio apiKey={apiKey} />}
        {activeTab === 'storyboard' && <StoryboardStudio apiKey={apiKey} />}
        {activeTab === 'scripts' && <ScriptsStudio apiKey={apiKey} />}
        {activeTab === 'reels'   && <ReelsStudio  apiKey={apiKey} />}
