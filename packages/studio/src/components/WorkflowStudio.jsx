  ...
  loading: () => (
    <div className="absolute inset-0 flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        <div className="w-12 h-12 border-4 border-white/5 border-t-[#d9ff3f] rounded-full animate-spin" />
...
// Normalize a workflow-run response into [{ id, type, value }] outputs.
// The api-outputs shape varies (array vs. object keyed by node id, and the value
// may live under value/url/*_url), so we handle all of them and, as a last resort,
// deep-scan the whole response for any media URL.
function inferOutputType(v) {
  if (typeof v !== "string") return "text";
  if (/\.(mp4|webm|mov|m4v)(\?|$)/i.test(v) || v.startsWith("data:video")) return "video_url";
  if (/\.(mp3|wav|ogg|m4a)(\?|$)/i.test(v) || v.startsWith("data:audio")) return "audio_url";
  if (/\.(png|jpe?g|webp|gif|avif|svg)(\?|$)/i.test(v) || v.startsWith("data:image")) return "image_url";
  if (/^https?:\/\//.test(v)) return "image_url"; // assume image for bare hosted urls
  return "text";
}
function extractOutputs(result) {
  if (!result) return [];
  const valOf = (o) =>
    o.value || o.url || o.output_url || o.image_url || o.video_url || o.audio_url ||
    o.result || o.output || (typeof o === "string" ? o : null);
  const items = [];
  const push = (id, val, type) => {
    if (val == null || val === "") return;
    items.push({ id: id || "output", value: val, type: type || inferOutputType(val) });
  };
  const raw = result.outputs ?? result.output_data ?? result.results ?? result.output ?? result.data;
  if (Array.isArray(raw)) {
    raw.forEach((o, i) => (typeof o === "string" ? push("output_" + i, o) : push(o.id || o.node_id || "output_" + i, valOf(o), o.type || o.output_type)));
  } else if (raw && typeof raw === "object") {
    Object.entries(raw).forEach(([k, o]) => (typeof o === "string" ? push(k, o) : o && typeof o === "object" ? push(o.id || k, valOf(o), o.type || o.output_type) : null));
  }
  // Fallback: deep-scan for any media URL anywhere in the response.
  if (!items.length) {
    const seen = new Set();
    const isMedia = (v) => typeof v === "string" && (/^https?:\/\/\S+\.(png|jpe?g|webp|gif|avif|mp4|webm|mov|mp3|wav)(\?|$)/i.test(v) || v.startsWith("data:image") || v.startsWith("data:video"));
    const walk = (o) => {
      if (o == null) return;
      if (typeof o === "string") { if (isMedia(o) && !seen.has(o)) { seen.add(o); push("output", o); } return; }
...
function WorkflowCard({ workflow, onClick, activeTab, onRename, onDelete }) {
  ...
  return (
    <div
      ...
      className="group relative aspect-[3/4] rounded-lg overflow-hidden cursor-pointer border border-white/5 bg-[#0a0a0a] transition-all hover:border-[#d9ff3f]/30 hover:scale-[1.02] shadow-2xl"
    >
      {/* Fallback tile (shown when there's no thumbnail, or the image URL is broken) */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#d9ff3f]/10 to-purple-500/10 flex items-center justify-center">
        <svg
          width="40"
          height="40"
          viewBox="0 0 24 24"
          ...
          {showOptions && (
            <div className="absolute top-10 right-0 w-32 bg-[#111] border border-white/10 rounded-lg shadow-2xl py-1 animate-in fade-in zoom-in duration-200">
              <button
                ...
                className="w-full px-4 py-2 text-left text-[11px] font-bold text-white/70 hover:text-[#d9ff3f] hover:bg-white/5 transition-colors flex items-center gap-2"
              >
      ...
      <div className="absolute inset-x-0 bottom-0 p-4">
        <div className="text-[10px] font-bold text-[#d9ff3f] uppercase tracking-wider mb-1 opacity-80">
          {workflow.category || "General"}
        </div>
        <h3 className="text-sm font-bold text-white truncate group-hover:text-[#d9ff3f] transition-colors">
...
export default function WorkflowStudio({ apiKey, isHeaderVisible = true, onToggleHeader }) {
    ...
    async function loadWorkflowDetails() {
      try {
        ...
        if (results[1].status === 'rejected' || results[2].status === 'rejected') {
          // Builder graph data (node schemas / definition) is only needed for the
          // BUILDER view. Don't block PLAY mode - just log it, so the run form works.
          console.warn("Builder graph data unavailable:", results[1].reason, results[2].reason);
        }
      } catch (err) {
  ...
  const handleRun = async (e) => {
    ...
    // The primary text prompt - reused to satisfy any extra required text root
    // nodes the api-inputs schema didn't expose (e.g. text2).
    const primaryPrompt =
      Object.entries(formData).find(([k, v]) => k.startsWith("text") && v)?.[1] ||
      Object.values(formData).find((v) => typeof v === "string" && v) ||
      "";

    const buildInputs = (extra = {}) => {
      const inputs = {};
    ...
    // Parse "Missing inputs for required root nodes: a, b" → fill them and retry.
    const fillMissing = (message) => {
      const m = /required root nodes?:\s*([^"}\]]+)/i.exec(message || "");
      if (!m) return null;
      const keys = m[1].split(/[,\s]+/).map((s) => s.trim()).filter(Boolean);
      if (!keys.length) return null;
      const extra = {};
      keys.forEach((key) => {
        if (key.startsWith("image")) extra[key] = { image_url: "" };
        else if (key.startsWith("video")) extra[key] = { video_url: "" };
        else extra[key] = { prompt: primaryPrompt || "Generate" };
      });
      return extra;
    };
    ...
    try {
      let data;
      try {
        data = await executeWorkflow(apiKey, selectedWorkflow.id, buildInputs());
      } catch (err) {
        const extra = fillMissing(err.message);
        if (!extra) throw err;
        // Retry once with the missing required root nodes auto-filled.
        data = await executeWorkflow(apiKey, selectedWorkflow.id, buildInputs(extra));
      }
      try { console.log("[workflow result]", data); } catch (_) {}
      setResult(data);
    } catch (err) {
  ...
  if (loading && !selectedWorkflow) {
    return (
      <div className="h-full flex items-center justify-center">
        <div className="animate-spin text-[#d9ff3f] text-3xl">◌</div>
      </div>
    );
  }
        ...
        {isHeaderVisible ? (
          <div className="flex-shrink-0 h-14 border-b border-white/5 flex items-center justify-between px-6 bg-black/40 z-30">
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-md transition-all ${
                      activeSubTab === "playground"
                        ? "bg-[#d9ff3f] text-black shadow-[0_0_15px_rgba(217,255,63,0.2)]"
            ...
            <div className="flex items-center gap-3">
              <span className="text-[11px] font-black text-[#d9ff3f] uppercase tracking-widest">
            ...
            <div className="flex bg-white/5 p-1 rounded-lg">
               <button
                 ...
                 className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-md transition-all ${
                   activeSubTab === "playground" ? "bg-[#d9ff3f] text-black" : "text-white/40"
                 }`}
               >
                 Play
               </button>
               <button
                 ...
                 type="button"
                 className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-md transition-all ${
                   activeSubTab === "builder" ? "bg-[#d9ff3f] text-black" : "text-white/40"
                 }`}
               >
                 Builder
               </button>
                ...
                <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                  <form onSubmit={handleRun} className="space-y-6">
                              ...
                              <div key={key} className="space-y-2">
                                ...
                                {prop.type === "string" && !prop.enum ? (
                                  <textarea
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-[#d9ff3f]/50 transition-colors min-h-[80px] resize-none"
                                    placeholder={
                                  ...
                                  <select
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-[#d9ff3f]/50 transition-colors"
                                  >
                                    {prop.enum.map((opt) => (
                                      <option
                                        key={opt}
                    ...
                    <button
                      ...
                      className="w-full py-4 bg-[#d9ff3f] text-black text-xs font-black uppercase tracking-[0.2em] rounded-xl hover:bg-white transition-all transform hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:grayscale shadow-[0_0_30px_rgba(217,255,63,0.15)] flex items-center justify-center gap-3 mt-8"
                    >
                      {isExecuting ? (
                        <>
              ...
              <div className="flex-1 overflow-y-auto p-8 lg:p-12 bg-[#050505] flex items-center justify-center min-h-[500px]">
                ...
                {isExecuting && (
                  <div className="flex flex-col items-center gap-6 animate-fade-in">
                    <div className="relative">
                      <div className="w-24 h-24 border-[3px] border-white/5 border-t-[#d9ff3f] rounded-full animate-spin shadow-[0_0_40px_rgba(217,255,63,0.1)]" />
                      <div className="absolute inset-0 flex items-center justify-center text-[#d9ff3f]">
                        <svg
                          width="32"
                          height="32"
                          viewBox="0 0 24 24"
                    ...
                    <div className="text-center space-y-2">
                      <div className="text-[10px] font-black text-[#d9ff3f] uppercase tracking-[0.3em] animate-pulse">
                        Running Pipeline
                      </div>
                ...
                {result && (
                  <div className="w-full max-w-4xl space-y-8 animate-fade-in-up">
                    ...
                    {extractOutputs(result).length === 0 && (
                      <div className="p-8 text-center text-white/40 text-[13px] border border-white/10 rounded-2xl bg-white/5">
                        The workflow completed but returned no media output.
                      </div>
                    )}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {extractOutputs(result).map((out, idx) => (
                        <div
                          key={idx}
                          className="group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-[#d9ff3f]/30 transition-all shadow-2xl"
                        >
                          {out.type === "image_url" ? (
                            <img
                              src={out.value}
                            ...
                            <div className="p-6 min-h-[200px] flex items-center justify-center italic text-white/60 break-words">
                              {out.value}
                            </div>
                          )}
                          ...
                          <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 to-transparent translate-y-full group-hover:translate-y-0 transition-transform">
                            <div className="flex items-center justify-between">
                              <span className="text-[10px] font-black text-[#d9ff3f] uppercase tracking-widest">
                                {out.id}
                              </span>
                              <a
                                href={out.value}
                                target="_blank"
                                rel="noreferrer"
                                className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center hover:bg-[#d9ff3f] hover:text-black transition-colors"
                              >
                                <svg
                                  width="14"
                                  height="14"
      ...
      <div className="max-w-7xl mx-auto w-full">
        <div className="flex flex-col gap-6 mb-12">
          <div className="flex items-end justify-between">
            ...
            <button
              ...
              className="px-6 py-3 bg-[#d9ff3f] text-black text-xs font-black uppercase tracking-widest rounded-lg hover:bg-white transition-all transform hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(217,255,63,0.3)] flex items-center gap-2"
            >
              <svg
                width="14"
                height="14"
        ...
        {loading ? (
          <div className="py-20 flex items-center justify-center">
            <div className="w-10 h-10 border-4 border-white/5 border-t-[#d9ff3f] rounded-full animate-spin" />
          </div>
        ) : (
          ...
          >
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-[#d9ff3f] uppercase tracking-widest">Workflow Name</label>
                <input
                  autoFocus
                  type="text"
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#d9ff3f]/50 transition-colors"
                />
              </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  type="submit"
                  className="flex-1 bg-[#d9ff3f] text-black px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-white transition-all transform hover:scale-105 active:scale-95"
                >
                  Save Name
                </button>
              </div>
